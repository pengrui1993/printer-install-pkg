...
void HttpRequest::readRequest(QTcpSocket* socket)
{
    ...
    if (!newData.isEmpty())
    {
        const auto decodedData = QString(newData.data());
        if(decodedData.startsWith("GET /?")){
            auto msg = decodedData.split("GET /?")[1];
            if(msg.contains(" ")){
                printerQuery = QByteArray::fromPercentEncoding(msg.split(" ")[0].toUtf8());
            }
        }
        qDebug("HttpRequest1: from %s: %s",qPrintable(socket->peerAddress().toString()),newData.data());
